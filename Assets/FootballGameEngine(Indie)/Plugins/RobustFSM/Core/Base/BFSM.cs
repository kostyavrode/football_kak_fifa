...
namespace RobustFSM.Base
{
    ...
    public class BFSM : IFSM
    {
        ...
        protected void ChangeState(Type type)
        {
            ...
            {
                throw new Exception("\n" + GetType().Name + " could not find '" + type.Name + "' in the state machine" + e.Message);
            }
        }
        ...
        public virtual T GetState<T>() where T : IState
        {
            ...
            {
                throw new Exception("\n" + GetType().Name + " could not find '" + typeof(T).Name + "' in the state machine" + e.Message);
            }
        }
        ...
        public void OnAnimatorExecuteIK(int layerIndex)
        {
            if (CurrentState != null)
                CurrentState.OnAnimatorExecuteIK(layerIndex);
        }

        public void OnAnimatorExecuteMove()
        {
            if (CurrentState != null)
                CurrentState.OnAnimatorExecuteMove();
        }

        public void OnControllerColliderCollide(ControllerColliderHit hit)
        {
            if (CurrentState != null)
                CurrentState.OnControllerColliderCollide(hit);
        }

        public void OnCollisionStart<T>(T collision)
        {
            if (CurrentState != null)
                CurrentState.OnCollisionStart(collision);
        }
